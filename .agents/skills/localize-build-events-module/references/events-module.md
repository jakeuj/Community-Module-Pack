...
發布前先重跑繁中 coverage 與 `test-official-event-timer.ps1 -Live`，以當下輸出更新頁面數字，不把 203／104／13 當永久常數。只提交預期檔案並推到 fork 的 `master`，不要推上游 `origin`，也不要為網站更新建立模組 tag 或 Release。推送後等待 Pages build 指向本次 commit 且狀態為 `built`，再從正式網域驗證品牌內容、Hero 資產、Upgrade Value、ArcDPS UI、最新版 Release 與 BHM 下載：
